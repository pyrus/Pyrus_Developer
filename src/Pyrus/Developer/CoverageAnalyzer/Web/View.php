...
class View
{
    ...
    public function getDatabase()
    {
        ...
        $output->startElement('html');
        {
            ...
            $output->startElement('body');
            {
                $output->writeElement(
                    'h2',
                    'Please enter the path to a coverage database'
                );
    ...
    public function logoutLink(XMLWriter $output)
    {
    ...
    public function TOC($sqlite)
    {
        $coverage = $sqlite->retrieveProjectCoverage();
        $this->renderSummary(
            $sqlite,
            $sqlite->retrievePaths(),
            false,
            $coverage[1],
            $coverage[0],
            $coverage[2]
    ...
    public function testTOC($sqlite, $test = null)
    {
        if ($test) {
    ...
    public function fileLineTOC($sqlite, $file, $line)
    {
        ...
        return $this->renderLineSummary(
            $file,
            $line,
            $sqlite->testpath,
            $source->getLineLinks($line)
        );
    }

    public function fileCoverage($sqlite, $file, $test = null)
    {
        if ($test) {
    ...
    public function render(SourceFile $source, $istest = false)
    {
        ...
        if (!$output->openUri('php://output')) {
            throw new Exception(
                'Cannot render ' . $source->shortName() . ', opening XML failed'
            );
        }
        ...
        if ($istest) {
            $output->writeElement(
                'title',
                'Code Coverage for ' . $source->shortName() . ' in ' .
                str_replace(
                    $source->testpath() . DIRECTORY_SEPARATOR,
                    '',
                    $istest
                )
            );
        ...
        $output->writeElement(
            ...
            '% (Covered lines / Executable lines)'
        );
        ...
        $output->writeRaw(
            '<p><strong>' .
            $total .
            '</strong> total lines, of which <strong>' .
            $info[1] .
            '</strong> are executable, <strong>' .
            $info[2] .
            '</strong> are dead and <strong>' .
            ($total - $info[2] - $info[1]) .
            '</strong> are non-executable lines</p>'
        );
        $output->writeRaw(
            '<p>Of those <strong>' .
            $info[1] .
            '</strong> executable lines there are <strong>' .
            $info[0] .
            '</strong> lines covered with tests and <strong>' .
            ($info[1] - $info[0]) .
            '</strong> lack coverage</p>'
        );
        $output->text("\n ");
        $output->startElement('p');
        $output->startElement('a');
    ...
    public function renderTestSummary(Aggregator $agg)
    {
        $output = new XMLWriter;
        if (!$output->openUri('php://output')) {
                throw new Exception(
    ...
    public function renderTestCoverage(Aggregator $agg, $test)
    {
