...
class Runner
{
    ...
    /**
     * Borrowed from the phpruntests project, under the PHP License
     */
    ...
    function runTests($tests)
    {
        ...
        if (count($failed)) {
            ...
            $mode = isset($this->_options['realtimelog']) ? 'a' : 'w';
            ...
            if (file_exists('run-tests.log') && !is_dir('run-tests.log')) {
    ...
    function run($file, $ini_settings = array(), $test_number = 1)
    {
        ...
        if ($this->windows && isset($section_text['RETURNS'])) {
            ob_start();
