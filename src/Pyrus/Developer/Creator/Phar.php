...
class Phar implements \Pyrus\Package\CreatorInterface
{
    /**
     * @var Phar
     */
    ...
    function __construct($path, $stub = false, $fileformat = \Phar::TAR, $compression = \Phar::GZ, array $others = null,
                         $releaser = null, \Pyrus\Package $new = null, $pkcs12 = null, $passphrase = '')
    {
        ...
        if ($pkcs12 && !extension_loaded('openssl')) {
            throw new \Pyrus\Developer\Creator\Exception('Unable to use ' .
        ...
        if (null !== $this->pkcs12) {
            ...
            if (!$pkcs) {
                throw new \Pyrus\Developer\Creator\Exception('Unable to process openssl key');
            }
            $private = openssl_pkey_get_private($cert['pkey']);
            if (!$private) {
                throw new \Pyrus\Developer\Creator\Exception('Unable to extract private openssl key');
            }
            ...
            if (true !== openssl_x509_checkpurpose($cert['cert'], X509_PURPOSE_SSL_SERVER,
                                                   \Pyrus\Channel\RemotePackage::authorities())) {
                throw new \Pyrus\Developer\Creator\Exception(
